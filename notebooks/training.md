...
Setting `loss_config.full_grid_collocation: true` bypasses sampling entirely for the data, physics, and grid-supervision losses: every `(t, h, w)` in the volume is used instead of a subset, and every other collocation knob above (`n_time`, `n_space`, the dense/uniform fractions) is ignored. This is only affordable on small grids/durations, since cost scales with the full volume rather than a fixed sample count -- off by default.
